  ...
  <div>
    ...
    >
      ...
      <el-table-column label="计划竣工日期" align="center" width="110px">
        <template slot-scope="scope">
          <span>{{ scope.row.endtime? $moment.unix(scope.row.endtime).format("YYYY-MM-DD"): ""}}</span>
        </template>
      </el-table-column>
  ...
  data() {
    ...
    return {
      ...
      addRules: {
        ...
        layers: [{ required: true, message: "请输入层数", trigger: "blur" }],
        engineering_cost: [
    ...
    getProjectDetail(id) {
      getDetail({ id }).then(res => {
        ...
        let starttime = this.$moment
          .unix(this.formData.starttime)
          .format("YYYY-MM-DD");
        let endtime = this.$moment
          .unix(this.formData.endtime)
          .format("YYYY-MM-DD");
